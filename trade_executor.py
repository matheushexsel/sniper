...
class TradeExecutor:
    ...
    async def calculate_edge(self, market: Dict, forecast_probability: float) -> Optional[Dict]:
        ...
        yes_tradeable = self.MIN_BUY_PRICE <= yes_price <= self.MAX_BUY_PRICE and edge_yes > 0

        ev_no = (1 - forecast_probability) * 1.0 - no_price
        edge_no = ev_no / no_price if no_price > 0 else -999
        no_tradeable = self.MIN_BUY_PRICE <= no_price <= self.MAX_BUY_PRICE and edge_no > 0
        ...
        if edge > self.MAX_BELIEVABLE_EDGE:
            logger.info(
                f"   ⚠️  Edge {edge:.1%} exceeds believable max — capping to {self.MAX_BELIEVABLE_EDGE:.0%} (model may be wrong)"
            )
